  ...
  Widget build(BuildContext context) {
    return Obx(() {
      if (controller.isLoading.value) {
        ...
        final paymentRefund = controller.architectSettings.value.needHelpPage?.paymentRefund;
        return Container(
          padding: const EdgeInsets.all(10),
          child: SingleChildScrollView(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: paymentRefund == null || paymentRefund.isEmpty
                  ? [const Text("No queries available.")]
                  : List.generate(paymentRefund.length, (index) {
                final section = paymentRefund[index];
                return Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    // if (index != paymentRefund.length - 1)
                    //   Center(
                    //     child: Container(
                    //       width: 300,
                    //       child: const Divider(
