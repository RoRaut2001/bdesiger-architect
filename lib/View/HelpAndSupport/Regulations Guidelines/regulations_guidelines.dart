...
class RegulationsGuidelines extends StatelessWidget {
  RegulationsGuidelines({super.key});
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      if (controller.isLoading.value) {
        ...
        final needHelp = controller.architectSettings.value.needHelpPage?.needHelp;
        return Container(
          padding: const EdgeInsets.all(10),
          child: SingleChildScrollView(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: needHelp == null || needHelp.isEmpty
                  ? [const Text("No queries available.")]
                  : List.generate(needHelp.length, (index) {
                final section = needHelp[index];
                return Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
