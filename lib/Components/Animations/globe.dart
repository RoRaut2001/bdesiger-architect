...
class _GlobeOfLogosState extends State<GlobeOfLogos>
  ...
  Timer? _autoRotationTimer; // Timer for auto-rotation delay
  ...
  void _handlePanEnd(DragEndDetails details) {
    ...
    // Start a timer to resume auto-rotation after 1 second
    _autoRotationTimer = Timer(const Duration(seconds: 1), () {
      if (!mounted) return;
  ...
  void dispose() {
    _controller.dispose();
    _autoRotationTimer?.cancel(); // Cancel the timer when disposing
    super.dispose();
  }
}
